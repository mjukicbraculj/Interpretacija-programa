...
def komplement(N):
    Q, Σ, Δ, q0, F = petorka(N)
    print(F, "stara zavrsna")
    zavrsna = set()
    for stanja in Q:
        if stanja not in F:
            zavrsna.add(stanja)
    F = zavrsna
    print(zavrsna, "su zavrsna")
    print(Q, "su stanja")

    return NedeterminističkiKonačniAutomat.definicija(
        Q, Σ, Δ, q0, F)
#
# def beskonačni_ciklus(N):
#     Q, Σ, Δ, q0, F = petorka(N)
#     for stanja in Q:
#         polazno = stanja
#         stanja_iz_polaznog = ε_zatvorenje(Δ, polazno)
#         if stanja not in stanja_iz_polaznog:
#             return True

def beskonačni_ciklus(N):
    Q, Σ, Δ, q0, F = petorka(N) # Δ je relacija, a e zatvorenje prima f-ju
    for stanje in Q:
        if stanje in ε_zatvorenje(N.funkcija_prijelaza, {stanje}):
            return True
    return False
...
def primjer():
    ...
    print(petorka(N1))
    Nkomplement = komplement(N1)
    print(petorka(Nkomplement))

    N = NedeterminističkiKonačniAutomat.definicija({'q1', 'q0'},
                                                   {'0', '1'},
                                                   {('q0', ε, 'q1'), ('q1', ε, 'q0')}, 'q1', {'q0'})
    print(beskonačni_ciklus(N))
    ...
    N1b = NedeterminističkiKonačniAutomat.definicija({1, 2, 3, 4}, {0, 1},
                                                     {(1, 0, 1), (1, 1, 1), (1, 1, 2), (2, 0, 3), (2, '', 3), (3, 1, 4),
                                                      (4, 0, 4), (4, 1, 4)}, 1, {4})
