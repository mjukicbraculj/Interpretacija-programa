...
class Buffer:
    ...
    def čitaj(self):
        self.redni_broj += 1
        if self.buffer: #if buffer is not empty return emelent on top
            self.zadnje = self.buffer.pop()
        else:       #else return next element in sequence
            self.zadnje = next(self.iterator, None) #None-default, returned when operator is exhausted
        # print(self.zadnje)
        return self.zadnje
